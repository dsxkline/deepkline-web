...
<template>
	<div class="nickname-container">
		<AppStatusBar/>
		<NavigationBar ref="navbar" :title="t('修改昵称')" />

		<div class="global-form p-6">
			<div class="form-item my-4">
				<el-input v-model="nickName" :placeholder="t('请输入昵称')" size="large" maxlength="20" minlength="3" clearable show-word-limit></el-input>
			</div>
			...
			<ul class="text-sm text-grey list-disc pl-4 *:py-1">
				<li>{{t('您今年还有{count}次机会修改',{count:useUserStore().user?.editNicknameTimes})}}次机会修改</li>
				<li>{{ t('格式：由语言字母开头，可包含数字、emoji、_ - .和空格，长度限制3-30个字符') }}</li>
				<li>{{ t('我们会审核您提交的昵称，过程需要一定时间，请耐心等待') }}</li>
				<li>{{ t('创建昵称时，请勿使用不雅用词、官方名称和其他交易平台的名称') }}</li>
			</ul>
