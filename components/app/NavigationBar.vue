...
<template>
    <div class="navbar flex items-center w-full h-[--nav-height] relative justify-between">
        <div class="navbar-left flex items-center h-full">
            <button v-if="!slots.left && !hideBack && !showClose" class="flex items-center px-3 h-full" @click="returnBack"><el-icon><ArrowLeftBold /></el-icon></button>
        ...
        <div class="navbar-right px-3 h-full flex items-center">
            <slot name="right" />
            <button v-if="showClose" class="flex items-center pl-3 h-full" @click="returnBack"><el-icon class="!w-6 !h-6"><Close class="!w-5 !h-5"/></el-icon></button>
        </div>
    </div>
</template>