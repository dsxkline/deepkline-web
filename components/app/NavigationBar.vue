...
<template>
	<div class="navbar flex items-center w-full h-[--nav-height] relative justify-between">
		<div class="navbar-left flex items-center h-full">
			<button v-if="!slots.left && !hideBack && !showClose" class="flex items-center px-3 h-full" @click="returnBack">
		...
		<div class="navbar-right px-3 h-full flex items-center">
			<slot name="right" />
			<button v-if="showClose" class="flex items-center pl-3 h-full" @click="returnBack">
